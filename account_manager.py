...
- firstmail.ltd / polosmail.com / любые на mx.firstmail.ltd
- Добавление новых — в IMAP_HOSTS словарь
"""
...
# IMAP настройки для разных почтовых сервисов
# По умолчанию — firstmail (он же полоска-мейл и т.д.)
IMAP_HOSTS = {
  "firstmail.ltd": {"host": "imap.firstmail.ltd", "port": 993},
  "polosmail.com": {"host": "imap.firstmail.ltd", "port": 993},
  "default": {"host": "imap.firstmail.ltd", "port": 993},
}


def get_imap_settings(email_address: str) -> dict:
  """Определяет IMAP настройки по домену почты."""
  domain = email_address.split("@")[-1].lower() if "@" in email_address else ""
  return IMAP_HOSTS.get(domain, IMAP_HOSTS["default"])
...
def load_accounts(filepath: str = ACCOUNTS_FILE) -> list:
  ...
  - Если строка содержит 4 части через : — новый формат
  """
  accounts = []
...
def fetch_tiktok_code_from_email(
  email_address: str,
  email_password: str,
  max_wait: int = 60
) -> Optional[str]:
  """
  Подключается к IMAP почты, ищет письмо от TikTok,
  ...
  imap_config = get_imap_settings(email_address)
  print(f"  [IMAP] Подключаюсь к {email_address} ({imap_config['host']}:{imap_config['port']})...")

  try:
      mail = imaplib.IMAP4_SSL(imap_config["host"], imap_config["port"], timeout=30)
          ...
          if status == "OK":
              ...
              if msg_ids:
                  ...
                  if status == "OK":
                      ...
                      print(f"  [IMAP] 6-значный код не найден, жду дальше...")
          ...
          try:
              ...
              if needs_code and acc["email"] and acc["email_password"]:
                  ...
                  if code:
                      ...
                      print(f"  [AccountManager] ❓ {acc['username']}: код не получен авто. Введи в браузере и нажми Enter...")
                      input()
                      await page.wait_for_timeout(3000)
