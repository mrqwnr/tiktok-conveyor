...
async with async_playwright() as p:
    browser = await p.chromium.launch(
        headless=False,  # обязательно visible — TikTok не любит headless
    ...
    for idx, acc in enumerate(accounts):
        ...
        try:
            ...
            # Проверяем — если появилась капча/код, ждём ручного ввода
            ...
            if "verify" in current_url.lower() or "code" in current_url.lower() or "captcha" in page_content.lower():
                ...
                input()  # ждём ручного ввода
                await page.wait_for_timeout(3000)
            
            # Проверяем — успешно ли залогинились
