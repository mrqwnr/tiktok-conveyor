...
2. Создаёшь accounts.txt (логин:пароль:почта:пароль_почты или логин/пароль построчно)
...
def main():
    """Главная функция — запускает выбранный этап конвейера."""
...
""")
    ...
    if mode in ("texts", "full"):
        print("✍️  ЭТАП 2: Генерация текстов")
    ...
    if mode in ("finalize", "full"):
        print("🎬  ЭТАП 3: Наложение текста на видео")
    ...
    if mode in ("login", "full"):
        print("🔑  ЭТАП 4: Авторизация аккаунтов")
        ...
        if not os.path.exists("accounts.txt"):
            ...
            print("логин:пароль:почта:пароль_почты")
            print("или построчно:")
            print("логин1\nпароль1")
            return
